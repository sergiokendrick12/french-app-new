...
export default function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(process.env.REACT_APP_API_URL || "https://portal.internationalfrenchacademy.org/api/enroll", {
        method: "POST",
...
<li><a href="https://portal.internationalfrenchacademy.org/admin" target="_blank" rel="noopener noreferrer" style={{color:"#4ade80",fontSize:"0.72rem",letterSpacing:"0.06em",textTransform:"uppercase",border:"1px solid rgba(74,222,128,0.4)",padding:"0.3rem 0.75rem",borderRadius:"4px",transition:"all 0.25s"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(74,222,128,0.15)"}} onMouseLeave={e=>{e.currentTarget.style.background="transparent"}}>⚙ Admin</a></li>
        </ul>
      ...
      <section id="pricing" style={{background:"#fff",padding:"5.5rem 2rem"}}>
        <div className="container">
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"1.5rem",gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))"}}>
            {[
              {name:lang==="en"?"Beginner":"Débutant",level:"DELF A1 – A2",price:"150,000",currency:"RWF",period:"",features:lang==="en"?["DELF A1: 150,000 RWF","DELF A2: 150,000 RWF","Expression écrite & orale","Exam preparation included"]:["DELF A1 : 150 000 RWF","DELF A2 : 150 000 RWF","Expression écrite & orale","Préparation aux examens"],color:"var(--navy)",popular:false},
{name:lang==="en"?"Certification":"Certification",level:"TCF / TEF",price:"200,000",currency:"RWF",period:lang==="en"?"/ starting from":"/ à partir de",features:lang==="en"?["TCF Québec: 200,000 RWF","TEF Québec: 200,000 RWF","TCF Canada: 200,000 RWF","TEF Canada: 200,000 RWF","Full support until cert"]:["TCF Québec : 200 000 RWF","TEF Québec : 200 000 RWF","TCF Canada : 200 000 RWF","TEF Canada : 200 000 RWF","Suivi jusqu'à certification"],color:"var(--gold)",popular:true},
{name:lang==="en"?"Advanced":"Avancé",level:"DALF / DILF / DFP",price:"200,000",currency:"RWF",period:lang==="en"?"/ starting from":"/ à partir de",features:lang==="en"?["DALF C1: 200,000 RWF","DALF C2: 200,000 RWF","DILF: 200,000 RWF","DFP: 250,000 RWF","1-on-1 coaching included"]:["DALF C1 : 200 000 RWF","DALF C2 : 200 000 RWF","DILF : 200 000 RWF","DFP : 200 000 RWF","Coaching individuel inclus"],color:"var(--mid-blue)",popular:false},
            ].map((p,i)=>(
              <FadeIn key={i} delay={i*100}>
      ...
      <section style={{background:"var(--navy)",padding:"5rem 2rem",textAlign:"center"}}>
        <div className="container">
          <div>
            ...
            <div style={{maxWidth:"320px",margin:"0 auto",borderRadius:"20px",overflow:"hidden",boxShadow:"0 30px 60px rgba(0,0,0,0.4)",border:"2px solid rgba(201,168,67,0.3)"}}>
              <video controls style={{width:"100%",height:"100%",display:"block",background:"#000",objectFit:"cover"}} poster="">
        ...
        <div className="container">
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:"1.5rem"}}>
            {[
              ...
              {m.featured && m.badge!==false && <div style={{position:"absolute",top:"0.75rem",right:"0.75rem",background:"var(--gold)",color:"var(--navy)",padding:"0.25rem 0.7rem",borderRadius:"20px",fontSize:"0.65rem",fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase"}}>{m.{lang==="en" ? (m.badgeText || "Director") : (m.badgeText === "Director" ? "Directeur" : m.badgeText)}}</div>}
                  </div>
                  <div style={{padding:"1.4rem 1.2rem"}}>
                    ...
                    {m.linkedin&&<div style={{padding:"0.5rem 0 1rem"}}><a href={m.linkedin} target="_blank" rel="noopener noreferrer" className="linkedin-btn" style={{display:"inline-flex",alignItems:"center",gap:"6px",background:"#0077b5",color:"#fff",padding:"6px 14px",borderRadius:"20px",fontSize:"0.75rem",fontWeight:600,textDecoration:"none",opacity:0,transition:"all 0.3s ease"}}><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="white" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.736 1.75-1.736 1.75.79 1.75 1.764-.783 1.736-1.75 1.736zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>LinkedIn</a></div>}
                  </div>
                </div>
              </FadeIn>
            ))}
                ...
                <div style={{borderRadius:"16px",overflow:"hidden",border:"1px solid var(--cream-dark)",boxShadow:"0 4px 24px rgba(13,27,42,0.08)",transition:"all 0.3s",background:"var(--cream)"}} onMouseEnter={e=>e.currentTarget.style.transform="translateY(-6px)"} onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}>
                  ...
                  <div style={{padding:"1.5rem"}}>
                    ...
                    <div style={{display:"flex",gap:"0.75rem"}}>
                      <a href="https://wa.me/250785632172?text=Hello%2C%20I%20want%20to%20order%20the%20book%3A%20" target="_blank" rel="noopener noreferrer" style={{flex:1,textDecoration:"none"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:"1rem"}}>
                {["A1","A2","B1","B2","C1","C2"].map((level,i)=>(
                  <div key={i} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(201,168,76,0.2)",borderRadius:"10px",padding:"1.2rem",textAlign:"center",transition:"all 0.3s",cursor:"default"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(201,168,76,0.1)";e.currentTarget.style.borderColor="var(--gold)";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.05)";e.currentTarget.style.borderColor="rgba(201,168,76,0.2)";}}>
                    ...
                    <a href={`https://wa.me/250785632172?text=Hello%2C%20I%20want%20to%20order%20the%20French%20${level}%20Guide`} target="_blank" rel="noopener noreferrer" style={{textDecoration:"none"}}>
      ...
      <footer>
        <div style={{maxWidth:"1080px",margin:"0 auto",padding:"3rem 2rem 1.5rem",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:"1.5rem",flexWrap:"wrap"}}>
          <div>
            ...
            <div style={{display:"flex",gap:"0.6rem",marginTop:"1rem"}}>
              {[{href:"https://www.facebook.com/internationalfrenchacademy",icon:<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="rgba(255,255,255,0.6)"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>},{href:"https://www.instagram.com/internationalfrenchacademy.rw",icon:<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.6)" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="rgba(255,255,255,0.6)" stroke="none"/></svg>},{href:`https://wa.me/250785632172`,icon:<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="rgba(255,255,255,0.6)"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>}].map((s,i)=>(
                <a key={i} href={s.href} target="_blank" rel="noopener noreferrer" style={{width:"34px",height:"34px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.25s"}} onMouseEnter={e=>e.currentTarget.style.borderColor="var(--gold)"} onMouseLeave={e=>e.currentTarget.style.borderColor="rgba(255,255,255,0.1)"}>{s.icon}</a>
              ))}
            </div>
          </div>
          <div>
            <div style={{color:"rgba(255,255,255,0.9)",fontSize:"0.8rem",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:"1rem"}}>Quick Links</div>
            {["about","vision","method","cert","pricing","place","contact"].map(id=>(
              <div key={id} style={{marginBottom:"0.5rem"}}>
                <button onClick={()=>document.getElementById(id)?.scrollIntoView({behavior:"smooth"})} style={{background:"none",border:"none",color:"rgba(255,255,255,0.4)",fontSize:"0.82rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",padding:0,transition:"color 0.2s",textTransform:"capitalize"}} onMouseEnter={e=>e.currentTarget.style.color="var(--gold)"} onMouseLeave={e=>e.currentTarget.style.color="rgba(255,255,255,0.4)"}>{id==="cert"?"Certifications":id==="pricing"?"Pricing":id.charAt(0).toUpperCase()+id.slice(1)}</button>
              </div>
            ))}
          </div>
          <div>
            <div style={{color:"rgba(255,255,255,0.9)",fontSize:"0.8rem",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:"1rem"}}>Certifications</div>
            {["DELF A1–B2","DALF C1–C2","TCF Québec","TEF Québec","TCF Canada","DILF","DFP"].map(c=>(
              <div key={c} style={{color:"rgba(255,255,255,0.4)",fontSize:"0.82rem",marginBottom:"0.5rem"}}>{c}</div>
            ))}
          </div>
          <div>
        ...
        <div className="footer-inner" style={{borderTop:"1px solid rgba(255,255,255,0.05)",paddingTop:"1.5rem"}}>
